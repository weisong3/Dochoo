...
public class RegisterInfoActivity extends ActionBarActivity
    ...
    private void initFragment(Intent intent) {
        if (intent != null && intent.getExtras() != null) {
            ...
            activateCode = extras.getString("activateCode");
        ...
        switch (currentRole) {
            case DOCTOR:
                if (info instanceof RegisterPhysicianInfo) {
                    isValid = mRegisterPresenter.checkPhysicianField(
                            info.getEmail(),
                            info.getPasswordUnhashed(),
                            info.getPasswordConfirm(),
                            ((RegisterPhysicianInfo) info).getFirstName(),
                            ((RegisterPhysicianInfo) info).getLastName(),
                            ((RegisterPhysicianInfo) info).getDegree(),
                            ((RegisterPhysicianInfo) info).getGender()
                    );
                }
                break;
            case CENTER:
                if (info instanceof RegisterCenterInfo) {
                    isValid = mRegisterPresenter.checkMedicalGroupField(
                            info.getEmail(),
                            info.getPasswordUnhashed(),
                            info.getPasswordConfirm(),
                            ((RegisterCenterInfo) info).getCenterName()
                    );
                }
                break;
            case PATIENT:
                if (info instanceof RegisterPatientInfo) {
                    isValid = mRegisterPresenter.checkPatientField(
                            info.getEmail(),
                            info.getPasswordUnhashed(),
                            info.getPasswordConfirm(),
                            ((RegisterPatientInfo) info).getFirstName(),
                            ((RegisterPatientInfo) info).getLastName(),
                            ((RegisterPatientInfo) info).getGender(),
                            ((RegisterPatientInfo) info).getDob());
                }
                break;
        }
        if(isValid){
    ...
    private void doRegister(RegisterUserInfo info) {
        ...
        switch (currentRole) {
            case DOCTOR:
                if (info instanceof RegisterPhysicianInfo) {
                    boolean isValid = mRegisterPresenter.checkPhysicianField(
                            info.getEmail(),
                            info.getPasswordUnhashed(),
                            info.getPasswordConfirm(),
                            ((RegisterPhysicianInfo) info).getFirstName(),
                    ...
                    if(isValid)
                        mRegisterPresenter.registerPhysician((RegisterPhysicianInfo) info, pushId, activateCode);
                }
                break;
            case CENTER:
                if (info instanceof RegisterCenterInfo) {
                    boolean isValid = mRegisterPresenter.checkMedicalGroupField(
                            info.getEmail(),
                            info.getPasswordUnhashed(),
                            info.getPasswordConfirm(),
                            ((RegisterCenterInfo) info).getCenterName()
                    ...
                    if(isValid)
                        mRegisterPresenter.registerMedicalGroup((RegisterCenterInfo) info, pushId, activateCode);
                }
                break;
            case PATIENT:
                if (info instanceof RegisterPatientInfo) {
                    boolean isValidInput = mRegisterPresenter.checkPatientField(
                            info.getEmail(),
                            info.getPasswordUnhashed(),
                            info.getPasswordConfirm(),
                            ((RegisterPatientInfo) info).getFirstName(),
                            ...
                            ((RegisterPatientInfo) info).getDob());
                    if (isValidInput)
                        mRegisterPresenter.registerPatient((RegisterPatientInfo) info, pushId, activateCode);
                }
                break;
        }
    }
    ...
    @Override
    public void onRegisterHalfSucceed() {
        Toast.makeText(this, getString(R.string.register_half_success_msg), Toast.LENGTH_SHORT).show();
        UserLogInActivity.startLogInActivity(this);
    }
